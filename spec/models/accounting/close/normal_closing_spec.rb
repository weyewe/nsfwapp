...
describe GroupLoan do
  ...
  context "withdraw remaining savings" do
    before(:each) do
      @group_loan.reload
      @group_loan.withdraw_compulsory_savings(
        :compulsory_savings_withdrawn_at => DateTime.now + 3.days + 3.hours
      )
    end
    
    it "should mark withdrawal as true " do
      @group_loan.is_compulsory_savings_withdrawn.should be_truthy 
    end
    
    it "should create Accounting posting for returning the cash" do
      TransactionData.where(:transaction_source_id => @group_loan.id, 
        :transaction_source_type => @group_loan.class.to_s,
